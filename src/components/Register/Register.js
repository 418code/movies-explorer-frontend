...
export default function Register(props) {
  const { values, handleChange, errors, isValid, resetForm } =
    useFormWithValidation(false, { name: "", email: "", password: "" });
  const intl = useIntl();
    ...
    >
      <div className="EnterForm__fields">
        <label className="EnterForm__label">
          {intl.formatMessage({ id: "name", defaultMessage: "Имя" })}
        </label>
        <input
          name="name"
          type="text"
          className={`EnterForm__input ${
            errors["name"] ? "EnterForm__input_type_error" : ""
          }`}
          placeholder={intl.formatMessage({
            id: "placeholder_name",
            defaultMessage: "Виталий",
          })}
          minLength="2"
          maxLength="30"
          value={values["name"] || ""}
        ...
        <label className="EnterForm__label">
          {intl.formatMessage({ id: "password", defaultMessage: "Пароль" })}
        </label>
        <input
          name="password"
          type="password"
