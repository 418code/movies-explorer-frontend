...
function App() {
  ...
  const [savedMoviesFlags, setSavedMoviesFlags] = useState(
    JSON.parse(localStorage.getItem("savedMoviesFlags")) || {}
  );
  const [checkedSaved, setCheckedSaved] = useState(
    JSON.parse(localStorage.getItem("checkedSaved")) || false
  );
  ...
  const [currentSearchMade, setCurrentSearchMade] = useState(
    JSON.parse(localStorage.getItem("currentSearchMade")) || false
  );
  ...
  const [currentShort, setCurrentShort] = useState(
    JSON.parse(localStorage.getItem("currentShort")) || false
  );
  const [currentText, setCurrentText] = useState(
    JSON.parse(localStorage.getItem("currentText")) || ""
  );
  ...
  const [popupContent, setPopupContent] = useState({
    success: false,
    message: intl.formatMessage({ id: "blank" }),
  });
  ...
  const filterMovies = useCallback((movies, searchString) => {
    ...
    const search = movies.filter(
      (obj) =>
        tokenize(obj.nameRU).filter(checker).length > 0 ||
        tokenize(obj.nameEN).filter(checker).length > 0
    );

    const shortSearch = search.filter(
      (obj) => parseInt(obj.duration) <= shortMovieMaxLength
    );
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "currentSearchMade",
      JSON.stringify(currentSearchMade)
    );
  ...
  useEffect(() => {
    const timer = setTimeout(
      () => setSavedPreloaderVisible(false),
      preloaderDelay
    );
    return () => clearTimeout(timer);
  ...
  useEffect(() => {
    if (popupContent.message !== "blank") setPopupOpened(true);
  }, [popupContent]);

  const handleCardSave = (card) => {
    if (savedMoviesFlags[card.movieId]) {
      api
        .deleteCard(savedMoviesFlags[card.movieId])
        .then((res) => {
          setSavedMovies((saved) =>
            saved.filter((movie) => movie.movieId !== card.movieId)
          );
  ...
  const getSavedMovies = () => {
    return api
      ...
      .catch((err) => {
        console.log(err);
        if (JSON.stringify(err).indexOf("404") === -1) showFailPopup();
        else {
          setCheckedSaved(true);
        }
      });
  ...
  const resetSavedText = useCallback(() => {
    setSavedText("");
  }, []);
    ...
    <CurrentUserContext.Provider value={{ currentUser, setCurrentUser }}>
      <div className="App body__element">
        ...
        <Routes>
          <Route
            path="/"
            element={<Landing menuClickHandler={menuClickHandler} />}
          />
          <Route
            path="/signup"
            ...
            element={
              <Private>
                <SavedMovies
                  ...
                  resetSavedText={resetSavedText}
                  resetSavedSearch={resetSavedSearch}
                  savedShort={savedShort}
                  setSavedShort={setSavedShort}
